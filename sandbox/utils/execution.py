...
def ensure_bash_integrity():
    ...
    try:
        if os.path.exists(bash_path) and file_md5(bash_path) == original_md5:
            return
        logger.warning("/bin/bash modified, trying to restore...")
        if not os.path.exists(internal_bash):
            logger.error("internal bash not found!")
            sys.exit(1)
        if file_md5(internal_bash) != original_md5:
            logger.error("internal bash modified!")
            sys.exit(1)
    ...
    try:
        ...
        for p in psutil.process_iter(["pid", "ppid"]):
            if p.ppid() == ppid:
                processes.append(p)
        if not processes:
            logger.warning("no child process found")
            return None
