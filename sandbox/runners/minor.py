...
def run_lua(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        ...
        return run_commands(None, f"lua {f.name}", tmp_dir, {}, args)


def run_r(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(mode="w", dir=tmp_dir, suffix=".R", delete=False) as f:
            f.write(args.code)

        return run_commands(None, f"Rscript {f.name}", tmp_dir, {}, args)


def run_perl(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(mode="w", dir=tmp_dir, suffix=".pl", delete=False) as f:
            f.write(args.code)

        return run_commands(None, f"perl {f.name}", tmp_dir, {}, args)


def run_d_ut(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(mode="w", dir=tmp_dir, suffix=".d", delete=False) as f:
            f.write(args.code)

        return run_commands(f"dmd {f.name} -unittest -of=test", "./test", tmp_dir, {}, args)


def run_ruby(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(mode="w", dir=tmp_dir, suffix=".rb", delete=False) as f:
            f.write(args.code)

        return run_commands(None, f"ruby {f.name}", tmp_dir, {}, args)


def run_scala(args: CodeRunArgs) -> CodeRunResult:
    ...
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        ...
        return run_commands(f"scalac {f.name}", f"scala {classname}", tmp_dir, {}, args)


def run_julia(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(mode="w", dir=tmp_dir, suffix=".jl", delete=False) as f:
            f.write(args.code)

        return run_commands(None, f"julia {f.name}", tmp_dir, {}, args)


def run_kotlin_script(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(mode="w", dir=tmp_dir, suffix=".kts", delete=False) as f:
            f.write(args.code)

        return run_commands(None, f"kotlin {f.name}", tmp_dir, {}, args)


def run_verilog(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        with tempfile.NamedTemporaryFile(mode="w", dir=tmp_dir, suffix=".sv", delete=False) as f:
            ...
            run_cmd = "vvp -n test.vvp"
        return run_commands(compile_cmd, run_cmd, tmp_dir, {}, args)


def run_lean(args: CodeRunArgs) -> CodeRunResult:
    ...
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        ...
        """
        return run_commands(None, "lake build", tmp_dir, {}, args, disable_pid_isolation=True)


def run_swift(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(mode="w", dir=tmp_dir, suffix=".swift", delete=False) as f:
            f.write(args.code)
        return run_commands(f"swiftc {f.name} -o test.out", "./test.out", tmp_dir, {}, args)


def run_racket(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(mode="w", dir=tmp_dir, suffix=".rkt", delete=False) as f:
            f.write(args.code)

        return run_commands(None, f"racket {f.name}", tmp_dir, {}, args)
