...
def get_cpp_rt_flags():
    ...
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        ...
        for flag in optional_flags:
            compile_res = run_command_bare(f"g++ {f.name} -o test {flag}", cwd=tmp_dir)
...
def run_python(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(mode="w", dir=tmp_dir, suffix=".py", delete=False) as f:
            f.write(args.code)

        return run_commands(None, f"python {f.name}", tmp_dir, get_python_rt_env("sandbox-runtime"), args, set_uid=config.sandbox.set_uid)


def run_pytest(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(mode="w", dir=tmp_dir, suffix=".py", delete=False) as f:
            f.write(args.code)

        return run_commands(None, f"pytest {f.name}", tmp_dir, get_python_rt_env("sandbox-runtime"), args)


def run_cpp(args: CodeRunArgs) -> CodeRunResult:
    flags = get_cpp_rt_flags()
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(mode="w", dir=tmp_dir, suffix=".cpp", delete=False) as f:
            f.write(args.code)

        return run_commands(f"g++ -std=c++17 {f.name} -o test {' '.join(flags)}", "./test", tmp_dir, {}, args)


def run_csharp(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        create_project_command = f"dotnet new console -o {tmp_dir}"
        run_command_bare(create_project_command, timeout=args.compile_timeout, cwd=tmp_dir)
        cs_file_path = os.path.join(tmp_dir, "Program.cs")
        with open(cs_file_path, "w") as cs_file:
            cs_file.write(args.code)

        return run_commands(None, f"dotnet run --project {tmp_dir}", tmp_dir, {}, args)


def run_go_test(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        ...
        return run_commands(None, f"go test {f.name}", tmp_dir, {}, args)


def run_go(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        source_dir = os.path.abspath(os.path.join(__file__, "../../../runtime/go"))
        for file in os.listdir(source_dir):
            shutil.copy2(os.path.join(source_dir, file), tmp_dir)
        ...
        return run_commands(f"go build -o out {f.name}", "./out", tmp_dir, {}, args)


def run_java(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        ...
        return run_commands(f"javac {cpargs} Main.java", f"java {cpargs} -ea Main", tmp_dir, {}, args)


def run_junit(args: CodeRunArgs) -> CodeRunResult:
    ...
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        ...
        return run_commands(f"javac -cp {cpargs} *.java", f"java -jar ./{junit_jar} --class-path {cpargs} --scan-class-path", tmp_dir, {}, args)


def run_nodejs(args: CodeRunArgs) -> CodeRunResult:
    ...
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        ...
        return run_commands(None, f"node {f.name}", tmp_dir, {}, args)


def run_typescript(args: CodeRunArgs) -> CodeRunResult:
    deps_dir = os.path.abspath(os.path.join(__file__, "../../../runtime/node"))
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        os.symlink(os.path.join(deps_dir, "node_modules"), os.path.join(tmp_dir, "node_modules"))
        ...
        return run_commands(None, f"tsx {f.name}", tmp_dir, {}, args)


def run_jest(args: CodeRunArgs) -> CodeRunResult:
    deps_dir = os.path.abspath(os.path.join(__file__, "../../../runtime/node"))
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        ...
        return run_commands(None, "npm run test", tmp_dir, {}, args)


def run_php(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        ...
        return run_commands(None, f"php -f {f.name}", tmp_dir, {}, args)


def run_rust(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(mode="w", dir=tmp_dir, suffix=".rs", delete=False) as f:
            f.write(args.code)

        return run_commands(f"rustc {f.name} -o test", "./test", tmp_dir, {}, args)


def run_bash(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(mode="w", dir=tmp_dir, suffix=".sh", delete=False) as f:
            f.write(args.code)

        return run_commands(None, f"/bin/bash {f.name}", tmp_dir, {}, args)
