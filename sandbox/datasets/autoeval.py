...
from sandbox.server.sandbox_api import RunCodeRequest, RunStatus, run_code
from sandbox.utils.common import ensure_json, generate_random_string
from sandbox.utils.extraction import default_extract_helper, extract_java_code, get_java_test_assets, remove_entripoints
...
class AutoEvalDataset(CodingDataset):
    ...
    async def get_prompts(cls, request: GetPromptsRequest) -> List[Prompt]:
        rows = await get_rows_in_table(request, cls.get_table_name(request.dataset), columns=["id", "labels", "content"])
    ...
    async def get_prompt_by_id(cls, request: GetPromptByIdRequest) -> Prompt:
        row = await get_row_by_id_in_table(request, cls.get_table_name(request.dataset), columns=["id", "labels", "content"])
    ...
    def _generate_single_prompt(cls, dataset: str, row: Dict[str, Any], config: TestConfig) -> Prompt:
        ...
        if template:
            ...
            prompt = Template(template).safe_substitute(question=question, fewshot=fewshot or "", context=context or "", locale=config.locale)
        else:
    ...
    def evaluate_single(cls, request: SubmitRequest) -> EvalResult:
        row = get_row_by_id_in_table(request, cls.get_table_name(request.dataset), columns=["test", "labels"])
        ...
        if execution_language == "jest" and "jest-report.json" in result.files:
            extra_result["jest_cases"] = parse_jest_cases(base64.b64decode(result.files["jest-report.json"]).decode("utf-8"))
